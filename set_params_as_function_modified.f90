subroutine set_params_as_function_modified (geometry,params)

  ! Routine to 
  !! @YW, modify to fix bug of only one block is used problem
...
!   geometry%v = 0.
  ...
  if(.NOT.params%f_varies_with_xyz)then
      ...
      ! make sure whole model run time has uplift specified. @YW on May 25, 2021
      if(params%time==0) then
          ...
          if(tinterval < params%tfinal)then
              print*, 
              print*, 'Warning!!! Uplift should be specified for all time intervals'    
                   
          elseif(tinterval > params%tfinal)then
              print*, 
              print*, 'Warning!!! There is overlap between time intervals for uplift specification'                                
          else
              print*, 
              print*, 'Great!!! Uplift uplift specification is all correct'                    
