...
!! @YW !!!! xarr ad yarr shouldn't exceed 3 elements by definition, why? At some restart settings ncount can exceed 3, so fix 
!this problem by make the xarr and yarr dynamic allocateble, May 24, 2021
  ...
  !if(params%istep.le.5)then
    ...
    do i=1,geometry%nnode
        ...
        if(bold(i).gt.0)then
            ...
            bcon=bold(i)
            corner=0
            allocate(xarr(geometry%nb(i)))
            allocate(yarr(geometry%nb(i)))
            xarr=0
            yarr=0
            ncount=0
            ...
            do j=1, geometry%nb(i)
                ...
                !if( bold(k).gt.0    .and. bold(k).eq.geometry%fix(k)  .and. bold(k).ne.bcon  .and. geometry%nb(i).lt.4  .and. (  xmax-xmin.gt.params%xc  .and.  ymax-ymin.gt.params%xc )  )then
                 if( bold(k).gt.0  .and. bold(k).eq.geometry%fix(k) .and. bold(k).ne.bcon .and. geometry%nb(i).lt.4)then ! The above condition seems sot check that the landscape domain is larger than Xc .... irrelevant I would say, and annoying given params%xc is now replaced by geometry%xc Odin 04/2020
                    corner=1
                    nbcon=bold(k)
                    exit
                endif
