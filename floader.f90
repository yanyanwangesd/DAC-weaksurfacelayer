...
!!!! this function reads the init.dac file, use the input numbers to assign parameters
...
! 1. Part
! do in advance some of "initialize_geometry"
! ...
...
! 2. Part
!  normal "initialize_parameters" tasks
! ...
...
params%rainfall_available=dread(s)            	! first orographic parameter (not fully implemented)
params%rainfall_minimum=dread(s)               	! second orographic parameter (not fully implemented)
...
! the diffusivity is doubled to be 2*diffusivity because 2*diffusivity appears everywhere in the divide solutions.
! The diffusivity given in init file is the true diffusivity value. @YWANG.
params%diffusivity=params%diffusivity*2.d0     	! double diffusivity as it always appears with a 2* in solution of divide (Eqn. 8 in Liran 2014)
...
params%plot_triangles=lread(s)                 	! flag to plot triangles
params%plot_receiver=lread(s)                 	! flag to plot node to receiver connections
params%plot_donors=lread(s)                    	! flag to plot node to donors connections
params%plot_no_receiver=lread(s)             	! flag to plot nodes with no receiver
params%plot_no_donors=lread(s)                 	! flag to plot nodes with no donor
params%write_discharge=lread(s)                	! flag to write discharge
params%write_stack_order=lread(s)              	! flag to write stack order
params%plot_catchment=lread(s)                 	! flag to plot catchments
params%plot_height=lread(s)                    	! flag to plot height contours
params%plot_precipitation=lread(s)             	! flag to plot precipitation contours
...
!!!!--------------------------- @YWANG, add regolith regime and flexure regime, Nov. 2021 ---------------------------

params%regolith_regime=lread(s)                 ! flag of whether regolith regime is used
params%flexure_regime=lread(s)                  ! flag of whether flexure regime is used
params%regolith_time=dread(s)                   ! the start time of regolith regime, in years
params%flexure_time=dread(s)                    ! the start time of flexure regime, in years
params%k_ratio=dread(s)                         ! the ratio of erodibiliry of hard bedrock (K_scaler1) to soft regolith
params%regolith_thickness=dread(s)              ! the finite thickness of the regolith layer before erosion, in meters
params%flexure_te=dread(s)                      ! the effective elastic thickness of the lithosphere, in meters
