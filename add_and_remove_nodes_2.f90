...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!! First stage: take care of all the 4 cases where a new node should added !!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  ! - - - - - - - - - - - - - - - - - - Case (1) prevent Delaunay flipping - - - - - - - - - - - - - - - - - 
  !This section is to add nodes on channels where the channel connect two nodes that are not neighbors (Delaunay flipping)
  ! @YW, this step add a node between the non-neighbour donor-receiver node pair as a correction of delaunay flip
  ...
  do i=1,nnold ! loop over the nodes
     if(geometry%fix(i).eq.0.and.network%receiver(i).ne.0)then 
        ...
        if(flag.eq.0)then !reciever is not a neighbor
           ...
           ! add one new node in between i and j
           ...
           geometry%dregolith(geometry%nnode)=(geometry%dregolith(i)+geometry%dregolith(j))/2.d0
  ...
  ! @YW, only the old nodes are checked, newly added nodes in case (1) are not checked, why?, why not add long-channel nodes before the delaunay flip correction?
  nadd_on_channel=0
  do i=1,nnold ! loop over the nodes
  ...
  ! - - - - - - - - - - - - - - - - - - Case (3) long divide - - - - - - - - - - - - - - - - - 
  ! this section add node on non-channel divides that are longer than a given distance
  ...
  ! (@YW, the if statement is never performed as long as any of the upper two cases are performed already, why?)
                 ...
                 if(geometry%surface_share(k,i).gt.0.d0)then  ! only continue if divide is closer to j than i
                    ...
                    if (l.gt.params%ldivmax) then ! add node on the connection
                       ...
                       if(xd.gt.params%xc)then  !  continue only if there is a fluvial segment to connection
                          ...
                          geometry%dregolith(geometry%nnode)=geometry%dregolith(i)
                          geometry%sediment_flux(geometry%nnode)=0.0d0
        ...
        if(geometry%fix(i).eq.0.and.network%ndon(i).eq.0.and.geometry%surface(i).lt.params%amin)then
           ...
           if(i.le.geometry%nnode)then
              do j=i,geometry%nnode
                 ...
                 geometry%dregolith(j)=geometry%dregolith(j+1)
...
!!!For that reason the donor receiver array needs to be updated
  ...
  ! - - - - - - - - - - - - - - - - - - Case (2) river segment too short - - - - - - - - - - - - - - - - - 
...
!!!For that reason the donor receiver array needs to be updated
  ...
  ! - - - - - - - - - - - - - - - - - - Case (3) node too close to boundary - - - - - - - - - - - - - - - - - 
