program DivideAndCapture
  ...
  if (params%regolith_regime) then
      call find_erodibility (geometry,params) ! use regolith regime
  endif
  ...
  call finetri (geometry, params,delaunay, network, landscape)
  ...
  write(100,'(e15.7, i5, i5, f15.3, i2, i2)') params%time, 0, 0, 0.d0, 0, 0
!   
  do while (params%time.lt.params%tfinal) ! start of time loop
     ...
     call captures_and_divides (geometry,network,params,delaunay,stack) ! find potential captures and adjusts network accordingly  
     ...
     call find_precipitation (geometry,params) ! orography (not fully working yet)
     !---Add regolith regime, June. 2021, by YWang
     if (params%regolith_regime) then
          call find_erodibility (geometry,params) ! use regolith regime
     endif
     ...
     !---Add flexural uplift, Oct. 2021, by YWang
     if (params%flexure_regime) then
         call flexure2D (geometry, params)
     endif
     ...
     if ((params%istep/params%freq)*params%freq.eq.params%istep) then	
        ...
        if (params%show_vtkfine) call VTKfine (geometry,delaunay,params,network,stack,landscape)
        if (params%ascii) call write_ascii(geometry,params,network)
        call find_hack(geometry,network,stack,params)
        call output_geometry(geometry,params,network,delaunay)
