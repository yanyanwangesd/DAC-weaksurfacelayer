subroutine reinit_geometry (geometry,params,network,delaunay)
  ...
  print*, 'Restart from previous run = ', params%time/1e3, ' [kyrs]'
  print*, 'Time new run after restart = ', params%tfinal/1e3, ' [kyrs]'
  
  params%tfinal = params%time+params%tfinal
  
  print*, 'Endtime (time previous run + time new run) = ',params%tfinal/1e3, ' [kyrs]'
  print*, '- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -'
  print*, '  '
  print*, '  '
...
!     print*, geometry%x(i),geometry%y(i),geometry%z(i)
  ...
  allocate (geometry%dregolith(geometry%nnode_max))
  allocate (geometry%flexure(geometry%nnode_max))
